...
You have two ways to create a skin. The simplest way is to create it in the {{< /highlight >}}layouts/{{< /highlight >}} directory. If you do, then you don’t have to worry about configuring Hugo to recognize it. The first place that Hugo will look for rules and files is in the {{< /highlight >}}layouts/{{< /highlight >}} directory so it will always find the skin.

Your second choice is to create it in a sub-directory of the {{< /highlight >}}themes/{{< /highlight >}} directory. If you do, then you must always tell Hugo where to search for the skin. It’s extra work, though, so why bother with it?

The difference between creating a skin in {{< /highlight >}}layouts/{{< /highlight >}} and creating it in {{< /highlight >}}themes/{{< /highlight >}} is very subtle. A skin in {{< /highlight >}}layouts/{{< /highlight >}} can’t be customized without updating the templates and static files that it is built from. A skin created in {{< /highlight >}}themes/{{< /highlight >}}, on the other hand, can be and that makes it easier for other people to use it.

The rest of this tutorial will call a skin created in the {{< /highlight >}}themes/{{< /highlight >}} directory a theme.

Note that you can use this tutorial to create a skin in the {{< /highlight >}}layouts/{{< /highlight >}} directory if you wish to. The main difference will be that you won’t need to update the site’s configuration file to use a theme.
...
Hugo translates Markdown files into HTML. By default, Hugo expects to find Markdown files in your {{< /highlight >}}content/{{< /highlight >}} directory and template files in your {{< /highlight >}}themes/{{< /highlight >}} directory. It will create HTML files in your {{< /highlight >}}public/{{< /highlight >}} directory. You can change this by specifying alternate locations in the configuration file.
